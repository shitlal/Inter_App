...
public class Validator implements Serializable {
	...
	private static final int PROPERTY_CONFIG_DIRECTORY = 0;
	...
	*/}
	...
	public static boolean validateSanctionedDates(Object bean, ValidatorAction validAction, Field field,
	     ...
	     System.out.println("Call validateSanctionedDate ............... SANCTION ..............................");
		//new PaymentServiceAPIMain().hitCorpApi(request) ;
		DynaActionForm dynaForm = (DynaActionForm) bean;
		String loanType = dynaForm.get("loanType").toString();
	    String fromValue = ValidatorUtil.getValueAsString(bean, field.getProperty());
	    String limitSanctDt = "";
	    if(request.getParameter("limitFundBasedSanctionedDate").toString()!=null  || !request.getParameter("limitFundBasedSanctionedDate").toString().equals("")) {
	    limitSanctDt = request.getParameter("limitFundBasedSanctionedDate");  // Added by dkr  
	    }
	    System.out.println("limitSanctDt--------------------------"+limitSanctDt);
		String toValue = null;
		...
		String restructConfirmation=dynaForm.get("restructConfirmation").toString();			
	  //    System.out.println("TC/WC/BO CAPITAL OR NOT loanType:::::: "+loanType +">>>>restructConfirmation "+restructConfirmation);	
		if (month >= 0 && month <= 2) {
			year--;
...
//			if(!loanType.equals("WC")) {
			...
			//  Added by DKR : relexcation for Maha bank 21-09-2021
				HttpSession session = request.getSession(false);
			    String userId = (String)session.getAttribute("USER_ID");
			    User user = (User) session.getAttribute(userId);
				String bankId = user.getBankId();
	//=================================  DKR ============================================================	  
				String guarantSanFromDt2Extension = "";//user.getBankId();
				String guarantSanToDt6Extension =  "";
				String guarantMLIID =  "";	
				String extensionSts="";
				Properties prop = null;				 
	       try {
				prop = ReadResourcePro.getProperties(request.getRealPath(PropertyLoader.PROPERTY_CONFIG_DIRECTORY+"/classes/AccountCodes.properties"));
				guarantSanFromDt2Extension = prop.getProperty("guarantSanFromDt2Extension");
				guarantSanToDt6Extension = prop.getProperty("guarantSanToDt6Extension");
				guarantMLIID = prop.getProperty("guarantMLIIDSancExtension");						
				extensionSts = prop.getProperty("extensionStatus");
			} catch (IOException e1) {
				e1.printStackTrace();
			}
				 String mliArr[] = guarantMLIID.split(",");
		    	 List<String> mliLst =  Arrays.asList(mliArr);
		    	 
    // ========================================================================================
		if((mliLst.contains(bankId)==true) && (extensionSts.equals("TRUE")==true)){				
	        ...
	        try
	        {
	            ...
	            Date d2 =   sdformat1.parse(guarantSanToDt6Extension); 
	             if(d1.compareTo(d2) <= 0)
	            {    
	                Date sanDate = dateFormat_1.parse(fromValue);
	                Date d6 = dateFormat_1.parse(guarantSanFromDt2Extension); 
	                if((loanType.equals("TC") || loanType.equals("BO")) && ((sanDate.before(d6) || sanDate.equals(d6)) && bankId.equals(guarantMLIID)))
	                {
	                	System.out.println(" Amount Sanctioned Date cannot be before++++++++++++++++++++++++++++++++++++++= "+ d6);
	             ...
	             //###########################################################################
	            	...
	            	System.out.println("validateSanctionedDates()>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"+e.getMessage());
					//e.printStackTrace();
				}
				
			}else {	
			  if ((loanType.equals("TC") || loanType.equals("BO")) && restructConfirmation.equals("N") && DateHelper.day1BeforeDay2(fromValue, toValue)){	
				  
				//  if(loanType.equals("TC") && restructConfirmation.equals("N") && DateHelper.day1BeforeDay2(fromValue, toValue)) {
					// Commented by DKR@11-11-2021
				  //	ActionError actionError = new ActionError("restructConfirmationMsg");
					//	errors.add("org.apache.struts.action.GLOBAL_ERROR", actionError);
				//	}
				/**  if(fromDateValue && toDateValue && DateHelper.day1BeforeDay2(fromValue, toValue)){	
			          ActionError actionMessage = new ActionError((new StringBuilder()).append("amountSanctionedDate cannot be before ").append(toValue).toString());
				      errors.add("org.apache.struts.action.GLOBAL_ERROR", actionMessage);
			         }*/
				  
			   } else if(loanType.equals("WC")) {
				   System.out.println("Treu checkbox===========   DKR ================== WECOJMEME  ==============");
			      ...
			      String dateInString =  fromValue;//"29/09/2019";//SanctionDate		
			      ...
			      if(sysFinalDate>=y2Date && y2Date<=sancTdate) {
			    	  System.out.println("Treu checkbox===========   DKR ================================"+restructConfirmation);					
					/**	if(restructConfirmation.equals("N") && DateHelper.day1BeforeDay2(fromValue, toValue)) {
							ActionError actionError = new ActionError("restructConfirmationMsg");
							errors.add("org.apache.struts.action.GLOBAL_ERROR", actionError);
						}else {
							errors.isEmpty();
			    	  ...
			    	  ActionError actionMessage = new ActionError("errors.required","Working Capital sanctioned date cannot be before "+ sdformat.format(p2year).toString()); //   DKR
	                  errors.add("org.apache.struts.action.GLOBAL_ERROR", actionMessage);
			      }
			  ...
			  // END
			//=================================================================================
			}// else close if not MHA  OK	       
		//}
	   }
		//========================================
		  System.out.println("Show Validation========= ok ========= BOTH FOR WC =========");
	         if(limitSanctDt!=""  && (loanType.equals("BO") || loanType.equals("CC"))) {
		         System.out.println("Show Validation================== BOTH FOR WC =============================================================");
				  SimpleDateFormat sdformat = new SimpleDateFormat("dd/MM/yyyy");
		      Calendar cal = Calendar.getInstance();
		      Date cdate = cal.getTime();
		      // get next year
		      ...
		      Integer sysFinalDate=Integer.valueOf(sFdate);     
		      String strP2Date = sdformat.format(p2year);
		      String dateInString =  limitSanctDt;//"29/09/2019";//SanctionDate		
		      Date sanctionDate = sdformat.parse(dateInString);
		      String fDate=sdformat.format(p2year); //CurrentDate+2 years
		      String splitFDate[]=strP2Date.split("/");
		      String mFdate=splitFDate[2]+splitFDate[1]+splitFDate[0];
	...
	public static boolean validateDates(Object bean, ValidatorAction validAction, Field field, ActionErrors errors,
		...
		try {
																...
																// not null
			{
					...
					String  strGuarDate=String.valueOf(guarDate);
                    System.out.println("strGuarDate...."+strGuarDate);
                    String[] npaDateArray=npaDateStr.split("/");
                    String[] guarArray=strGuarDate.split("/");
                    ...
                    int dcnt=0;
                           ...
                           if(Integer.parseInt(npaDateArray[2])<Integer.parseInt(guarArray[2])) {
                                  ycnt++;
                                  cnt++;
                           }
                           if(Integer.parseInt(npaDateArray[1])<Integer.parseInt(guarArray[1])) {
                                  mcnt++;
                                  cnt++;
                           }
                           if(Integer.parseInt(npaDateArray[0])<Integer.parseInt(guarArray[0])) {
                                  dcnt++;
                                  cnt++;
                           }
                    System.out.println("guarArray cnt ....."+cnt);
//                    if(cnt==2 && ycnt==0) {
...
//                    }
	...
	public static boolean checkCreditAmtAndSanctionDateForTC(Object bean, ValidatorAction validAction, Field field,
        ...
        try
        {
            ...
            if(dynaForm.get("creditGuaranteed") != null && dynaForm.get("amountSanctionedDate") != null && ((Double)dynaForm.get("creditGuaranteed")).doubleValue() > 0.0D)
            {
              ...
              /**  if(!sanctionDateAmountValidationAll("tc", dateAndAmountDtl, basedOnCondition))   // COMMENTED BY DKR 11-11-2021
                {
                	System.out.println("creditAmountAndSanctionDateVali--------------1-------- Error--");
            ...
            if(dynaForm.get("creditFundBased") != null)
            {
               ...
               // basedOnCondition.put("none", dynaForm.get("none"));
                ...
                if(!sanctionDateAmountValidationAll("wc", dateAndAmountDtl, basedOnCondition))
                {
                	...
                	System.out.println("creditAmountAndSanctionDateValiWC--------------2-------- Error--");
	        ...
	        try
	        {
	            if(dynaForm.get("creditGuaranteed") != null && dynaForm.get("amountSanctionedDate") != null && ((Double)dynaForm.get("creditGuaranteed")).doubleValue() > 0.0D)
	            {
	                ...
	                if(!sanctionDateAmountValidationAll("tc", dateAndAmountDtl, basedOnCondition))
	                {
	                	...
	                	System.out.println("creditAmountAndSanctionDateVali--------------3---- TC ---- Error--");
	        ...
	        try
	        {
	            ...
	            if(dynaForm.get("wcFundBasedSanctioned") != null || dynaForm.get("wcNonFundBasedSanctioned") != null)  //&&2021
	            {
	                ...
	                if(!sanctionDateAmountValidationForWCEnhancement("wc", dateAndAmountDtl, basedOnCondition))
	                {
	                	...
	                	System.out.println("creditAmountAndSanctionDateValiWCEnhancement--------------5-------- Error--");
	...
	*/}
	...
	public static boolean validateExistGreenUnit(Object bean, ValidatorAction validAction, Field field,
		...
		// Commented by DKR 11-11-2021
			//if (creditGuaranteedVal >= 10000000D && creditGuaranteedVal <= 20000000D && datFlag4UI1) {
				if (creditGuaranteedVal >= 10000000D && creditGuaranteedVal <= 20000000D) {
				session.removeAttribute("dblockUI");
		...
		if (sessionLoanType.equals("WC") && (creditFundBasedVal != 0.0D) && (limitFundbasedSanctionedDateVal != null)
				...
				// Commented by DKR 11-11-2021        && (datFlag4UI2 || datFlag4UI3)) {
	...
	//	Date amountSanctionedDate = (Date) dynaForm.get("amountSanctionedDate");
		...
		Date currDate = new Date();
		System.out.println("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@checkTerminationSanctionDate@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"+currDate.before(expiryDate));
	//	System.out.println(amountSanctionedDate + "~~~~~  CHECK DKR TERMINATION DATE ~~~~ DR ~~~~~  DDDDDDDDDDDDDDDDDDDD DDDDDDDDDDDDDDDDDDDDDD DDDDDDDDDDDDDDDDDDDDDDDD~~~~---------------------~~~~~~~~~~~~~" + expiryDate);
     /**	if (amountSanctionedDate.compareTo(expiryDate) > 0) {
			System.out.println(amountSanctionedDate	+ "~~~~~~~~~ DR ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~  >  greater ~~~~~~~~~~" + expiryDate);
			ActionError actionError = new ActionError("sanctionExpiryDtErr");
			errors.add("org.apache.struts.action.GLOBAL_ERROR", actionError);
		}
		*/
	//	System.out.println(currDate	+ "~~~~~~~~~ DR ~~~  0NaN/0NaN/NaN ~~~~~~~~" + expiryDate+" currDate.compareTo(expiryDate)"+currDate.compareTo(expiryDate)+"   currDate.after(expiryDate)"+currDate.after(expiryDate));
   //	System.out.println("currDate.compareTo(expiryDate) > 0   "+ (currDate.compareTo(expiryDate) > 0)+"==============================="+ expiryDate	+ "~~~~~~~~~ Expiry ~~~~~~~~~~ 87 ~~~~~~~~CURRENT~~~~~~~~" +new SimpleDateFormat("dd/MM/yyyy").format(currDate)+":::::::::::::"+currDate.compareTo(expiryDate));
	    if (currDate.before(expiryDate)==false && null!=expiryDate) {
		//	System.out.println(currDate	+ "~~~~~~~~~ DR ~~~~~~~~~~~" + expiryDate+" currDate.compareTo(expiryDate)"+currDate.compareTo(expiryDate)+"   currDate.after(expiryDate)"+currDate.after(expiryDate));
			ActionError actionError = new ActionError("expiryDtErr");
			errors.add("org.apache.struts.action.GLOBAL_ERROR", actionError);
		}
		return errors.isEmpty();
	}
